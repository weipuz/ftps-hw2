...
phrase = namedtuple("phrase", "english, scores, computed_score")
def TM(filename, k, weights):
	sys.stderr.write("Reading translation model from %s...\n" % (filename,))
	#weights = matrix(weights)
	tm = {}
	for line in open(filename).readlines():
		(f, e, scores) = line.strip().split(" ||| ")
		scores = matrix(map(float,scores.strip().split(" ")))
		computed_score = weights * scores.T
		tm.setdefault(tuple(f.split()), []).append(phrase(e, scores, float(computed_score)))
	for f in tm: # prune all but top k translations
		tm[f].sort(key=lambda x: -x.computed_score)
		del tm[f][k:] 
	return tm
...
class LM:
	def __init__(self, filename):
		...
		self.table[("<s>",)] = ngram_stats(-99,-1.640621)
		
		fileName, fileExtension = os.path.splitext(filename)
		file = gzip.open(filename) if (fileExtension == '.gzip' or fileExtension == '.gz') else open(filename)
		for line in file:
